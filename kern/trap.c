...
void trap_init_percpu(void)
{
	...
	struct Taskstate* local_ts = &(thiscpu->cpu_ts);
	local_ts->ts_esp0 = KSTACKTOP - id *(KSTKSIZE + KSTKGAP);
	local_ts->ts_ss0 = GD_KD;
	local_ts->ts_iomb = sizeof(struct Taskstate);

	// Initialize the TSS slot of the gdt.
	gdt[(GD_TSS0 >> 3) + id] = SEG16(STS_T32A, (uint32_t)(local_ts),
...
void page_fault_handler(struct Trapframe *tf)
{
	...
	uintptr_t UXSTACKDOWN = UXSTACKTOP - PGSIZE, cur_top;
	struct UTrapframe* etf;
	if(tf->tf_esp >= UXSTACKDOWN && tf->tf_esp < UXSTACKTOP){
		cur_top = tf->tf_esp-4;
	}
	else{
		cur_top = UXSTACKTOP;
	...
	etf = (struct UTrapframe*)(cur_top - sizeof(struct UTrapframe));
	user_mem_assert(curenv, etf, sizeof(struct UTrapframe), PTE_W|PTE_U);

	etf->utf_fault_va = fault_va;
	etf->utf_err = tf->tf_err;
	etf->utf_regs = tf->tf_regs;
	etf->utf_eip = tf->tf_eip;
	etf->utf_eflags = tf->tf_eflags;
	etf->utf_esp = tf->tf_esp;

	// ensure environment returns to pfentry.S with tf stack
	curenv->env_tf.tf_eip = (uintptr_t)curenv->env_pgfault_upcall;
	curenv->env_tf.tf_esp = (uintptr_t)etf;
